/**
 * ═══════════════════════════════════════════════
 * VBF Dashboard Module — dashboard.js
 * ═══════════════════════════════════════════════
 * Admin dashboard with statistics, charts,
 * upcoming inspections, and quick actions.
 * 
 * Dependencies: Chart.js (CDN), app.js (API_BASE_URL, currentToken)
 */
        ...
        analyticsRow.innerHTML = `
            ...
            <div class="dash-chart-card panel-glass">
                ...
                <div class="dash-pass-rate">
                    <div class="dash-pass-circle" style="--rate: ${stats.result_stats.pass_rate}%">
                        <span class="dash-pass-number">${stats.result_stats.pass_rate}%</span>
                        <span class="dash-pass-label">Átmenési arány</span>
                    </div>
                    <div class="dash-pass-details">
                        <div class="dash-pass-item ok">✅ Megfelelt: <strong>${stats.result_stats.passed}</strong></div>
                        <div class="dash-pass-item fail">❌ Nem felelt meg: <strong>${stats.result_stats.failed}</strong></div>
                        <div class="dash-pass-item total">📏 Összes mérés: <strong>${stats.result_stats.total_measurements}</strong></div>
                    </div>
                </div>
            </div>
        `;
        ...
        if (inspections) {
            ...
            inspSection.innerHTML = `
                ...
                <div class="dash-insp-summary">
                    <span class="dash-badge danger">${inspections.total_overdue} lejárt</span>
                    <span class="dash-badge warning">${inspections.total_upcoming} közelgő (90 nap)</span>
                </div>
                ...
                allItems.forEach(item => {
                    ...
                    row.innerHTML = `
                        <div class="dash-insp-info">
                            ...
                            <span class="dash-insp-meta">${item.customer_name} · ${item.report_type.toUpperCase()} · ${item.otsz_class}</span>
                        </div>
...
// Auto-init when the dashboard tab is clicked
